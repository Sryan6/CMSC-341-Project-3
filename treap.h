...
private:
  ...
  // Rotate right node up to the head position using AVL rotation
  void rotateLeft();

  // Rotate left node up to the head position using AVL rotation
  void rotateRight();

  //This function allows a recursive call to free all the nodes in a treap
  void clear();

  //A helper function that compartmentalizes the copying
  //for the copy constructor and assignment operator
  TreapNode* copy(TreapNode* copy) const ;
} ;
